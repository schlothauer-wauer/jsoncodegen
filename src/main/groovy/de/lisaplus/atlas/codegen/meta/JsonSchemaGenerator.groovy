 ...
 */
class JsonSchemaGenerator extends SingleFileGenarator {
    private static final Logger log=LoggerFactory.getLogger(JsonSchemaGenerator.class)

    void initTemplate() {
        template = createTemplateFromResource('templates/meta/json_schema.txt',TemplateType.GString)
    }
    ...
    String getDestFileName(Model dataModel, Map<String, String> extraParameters, Type currentType=null) {
        if (extraParameters.destFileName) {
            ...
            return "${string2Name(dataModel.title,false)}_schema.json"
        }
    }
